...
# nginx-unprivileged (not nginx:alpine): runs as the built-in non-root
# "nginx" user out of the box, with /var/cache/nginx, /var/run etc already
# writable by it. Plain nginx:alpine's master process runs as root because
# binding port 80 requires it — listening on 8080 instead avoids that.
